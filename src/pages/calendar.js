...
export default function BookingCalendar() {
  ...
  const handleBookingSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        enqueueSnackbar("Booking saved successfully!", { variant: "success" });
      ...
      enqueueSnackbar("Server error. Contact Hardik.", { variant: "error" });
    } finally {
      setModalOpen(false);
      setLoadingBTN(false);
    }
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <Box sx={{ width: "98.5vw", height: "100vh", p: 2, display: "flex", flexDirection: "column", bgcolor: "#f4f4f4" }}>
        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 2 }}>
          ...
          <Box>
            <Button variant="contained" onClick={() => setCurrentMonth(currentMonth.subtract(1, "month"))} sx={{ mx: 1 }}>
              ← Prev
            </Button>
            <Button variant="contained" onClick={() => setCurrentMonth(currentMonth.add(1, "month"))} sx={{ mx: 1 }}>
              Next →
            </Button>
          </Box>
        </Box>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "5px", textAlign: "center", fontWeight: "bold", mb: 1 }}>
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
            <Typography key={day} sx={{ fontSize: "16px", bgcolor: "#ddd", py: 1, borderRadius: "4px" }}>
              {day}
            </Typography>
          ))}
        </Box>
        ...
        <Box sx={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "5px", flex: 1 }}>
          {loading ? (
            ...
            getDaysInMonth().map((date, index) => {
              ...
              return (
                <Box key={dateStr} sx={{ border: "1px solid #ddd", borderRadius: "8px", display: "flex", flexDirection: "column", height: "100px", position: "relative", cursor: isAdmin ? "pointer" : "default" }} onClick={isAdmin ? () => handleOpenModal(date) : null}>
        ...
        <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
          <Box sx={{ bgcolor: "white", p: 3, mx: "auto", my: "20vh", width: "400px", borderRadius: "8px" }}>
            ...
            <Box>
            <Button variant="contained" disabled={loadingBTN} onClick={handleBookingSubmit} sx={{ mt: 2 }}>Save Booking</Button>
            <Button variant="outlined" disabled={loadingBTN} color="error" onClick={handleDeleteBooking} sx={{ mt: 2, ml: 1 }}>Clear Booking</Button>
            </Box>
          </Box>
        </Modal>
      </Box>
